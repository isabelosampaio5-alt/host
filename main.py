...
COOKIES_SENT_FILE = "cookies_enviados.json"  # Rastreia quais cookies cada usuário já recebeu
...
def load_sent_cookies():
    """Carrega o registro de cookies já enviados para cada usuário."""
    if not os.path.exists(COOKIES_SENT_FILE):
        return {}
    try:
        with open(COOKIES_SENT_FILE, "r") as f:
            return json.load(f)
    except:
        return {}


def save_sent_cookies(sent_data):
    """Salva o registro de cookies enviados."""
    try:
        with open(COOKIES_SENT_FILE, "w") as f:
            json.dump(sent_data, f, indent=2)
    except Exception as e:
        logger.error(f"Erro ao salvar registro de envios: {e}")


def mark_cookie_as_sent(user_id, cookie_id):
    """Marca um cookie como enviado para um usuário específico."""
    sent_data = load_sent_cookies()
    user_key = str(user_id)

    if user_key not in sent_data:
        sent_data[user_key] = []

    if cookie_id not in sent_data[user_key]:
        sent_data[user_key].append(cookie_id)
        save_sent_cookies(sent_data)


def get_unsent_cookie(user_id):
    """
    Retorna um cookie aleatório que o usuário ainda não recebeu.
    Retorna None se todos os cookies já foram enviados para esse usuário.
    """
    ...
    # IDs de cookies já enviados para este usuário
    sent_ids = sent_data.get(user_key, [])

    # Filtra cookies que ainda NÃO foram enviados
    unsent = [c for c in saved if c['id'] not in sent_ids]

    if not unsent:
        return None  # Todos já foram enviados
...
async def cmd_list_cookies(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Lista APENAS a quantidade de cookies salvos.
    Uso: /cookies
    """
    saved = load_saved_cookies()
    total = len(saved)
    ...
    if total == 0:
        ...
        msg = (
            f"<b>🍪 Banco de Cookies Netflix</b>\n\n"
...
async def cmd_netflix(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Envia um cookie aleatório que o usuário AINDA NÃO recebeu.
    Uso: /netflix
    """
    user_id = update.effective_user.id
    save_user(user_id)

    # Busca um cookie não enviado para este usuário
    chosen = get_unsent_cookie(user_id)
    ...
    # Marca como enviado para este usuário
    mark_cookie_as_sent(user_id, chosen['id'])

    # Monta a mensagem com o cookie
...
async def cmd_cookie_detail(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Mostra detalhes de um cookie específico (admin)."""
    user_id = update.effective_user.id
    if user_id != ADMIN_ID:
        await update.message.reply_text("❌ Apenas o admin pode usar este comando.")
        return
    ...
    if not context.args:
        await update.message.reply_text("❌ Use: <code>/cookieinfo [ID]</code>", parse_mode='HTML')
        return
...
async def cmd_delete_cookie(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text(
            "❌ Use: <code>/delcookie [ID]</code> ou <code>/delcookie all</code>",
            parse_mode='HTML'
        )
        return
    ...
    if context.args[0].lower() == 'all':
        ...
        save_sent_cookies({})  # Limpa também o registro de envios
...
def main():
    ...
    # Comando principal para usuários
    app.add_handler(CommandHandler("cookies", cmd_list_cookies))  # Mostra quantidade
    app.add_handler(CommandHandler("netflix", cmd_netflix))  # Envia cookie único

    # Comandos de gerenciamento (admin)
    app.add_handler(CommandHandler("addcookie", cmd_add_cookie))
    app.add_handler(CommandHandler("cookieinfo", cmd_cookie_detail))  # Detalhes admin
    ...
    print("  /start - Iniciar")
    print("  /cookies - Ver quantidade de cookies disponíveis")
    print("  /netflix - Receber um cookie aleatório (nunca recebido antes)")
    print("\n📋 Comandos para ADMIN:")
    print("  /addcookie [cookie] - Adicionar cookie")
    print("  /cookieinfo [ID] - Ver detalhes de um cookie")
    print("  /editcookie [ID] [novo] - Editar cookie")
    print("  /delcookie [ID] - Excluir cookie")
    print("  /delcookie all - Excluir todos")
    print("  /all - Broadcast")
    app.run_polling()
